...
export default defineEventHandler(async (event) => {
    ...
    await authMiddleware(event);
        ...
        switch (data?.type) {
            case 0:
                if (!user.$.friendsCount)
                    return await event.context.connector.error(event, -2, "No friends")
            ...
            case 1:
                if (!user.$.blacklistedUsers?.length)
                    return await event.context.connector.error(event, -2, "No blacklisted users")
        ...
        if (!users?.length)
            return await event.context.connector.error(event, -2, "No users")
