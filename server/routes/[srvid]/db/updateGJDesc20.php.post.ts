...
export default defineEventHandler(async (event) => {
    ...
    await authMiddleware(event);
        ...
        if (!level.validate())
            return await event.context.connector.error(event, -1, "Invalid level description")

        await level.commit()

        return await event.context.connector.success(event, "Level description updated successfully")
    }
)
